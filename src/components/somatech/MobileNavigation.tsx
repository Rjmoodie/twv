...
const MobileNavigation = ({
  ...
  const showUpgradeNudge =
    ...
    !subscription.userProfile?.role?.includes('admin');
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              <div className="text-sm font-semibold text-foreground">SomaTech</div>
        ...
        {/* Upgrade nudge */}
        {showUpgradeNudge && onRequestUpgrade && (
          <div className="px-2 pb-3 pt-2 border-t border-border/60 shrink-0" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
            <UpgradeNudge
              onUpgrade={() => { navigate('/pricing'); onClose(); }}
            />
          </div>
        )}
      </div>
