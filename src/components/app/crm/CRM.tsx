...
const relationshipLabel = (value: ContactKind) => value === 'investor'
  ? 'Investor partner · capital'
  : value === 'client'
    ? 'Investor partner · project'
    : pretty(value);
...
export default function CRM() {
  ...
  return <div className="operations-module space-y-6 pb-24 lg:pb-8">
    <header className="operations-module__header flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className="mb-2 flex gap-2"><span className="operations-eyebrow">Investor & project relationships</span>{hasPersona('project_manager') && !canCreate && <Badge variant="secondary">Assigned projects only</Badge>}</div><h1 className="operations-title text-4xl tracking-tight">Relationship desk</h1><p className="mt-1 text-muted-foreground">A clear record of every partner, conversation, commitment, and next action.</p></div>{canCreate && <Button onClick={() => setCreateOpen(true)}><Plus className="mr-2 h-4 w-4" />Add relationship</Button>}</header>
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      <Stat icon={Users} label="All relationships" value={contacts.length} active={filter === 'all'} onClick={() => setFilter('all')} />
      <Stat icon={Building2} label="Capital partners" value={contacts.filter((contact) => contact.kind === 'investor').length} active={filter === 'investor'} onClick={() => setFilter('investor')} />
      <Stat icon={UserRound} label="Project partners" value={contacts.filter((contact) => contact.kind === 'client').length} active={filter === 'client'} onClick={() => setFilter('client')} />
    ...
    {contactsQuery.isLoading ? <div className="flex min-h-[260px] items-center justify-center"><Loader2 className="h-7 w-7 animate-spin" /></div> : contacts.length === 0 ? <Card className="operations-empty-state"><CardContent className="flex min-h-[380px] flex-col items-center justify-center p-8 text-center"><div className="operations-empty-state__icon"><Users className="h-9 w-9" /></div><span className="operations-eyebrow mt-5">Relationship ledger</span><h2 className="operations-title mt-2 text-3xl">Build the relationship record</h2><p className="mt-2 max-w-md text-sm text-muted-foreground">{canCreate ? 'Add an investor partner, lender, broker, vendor, or project professional and connect the relationship to an asset.' : 'Relationships connected to your assigned projects will appear here.'}</p>{canCreate && <Button className="mt-6" onClick={() => setCreateOpen(true)}><Plus className="mr-2 h-4 w-4" />Add first relationship</Button>}</CardContent></Card> : filtered.length === 0 ? <Card><CardContent className="p-10 text-center"><p className="font-medium">No relationships match this view.</p><Button variant="link" onClick={() => { setFilter('all'); setSearch(''); }}>Clear filters</Button></CardContent></Card> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{filtered.map((contact) => {
...
const CreateContactDialog = ({ open, onOpenChange, organizationIds, projects, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; organizationIds: string[]; projects: PortfolioChoice[]; onCreated: () => Promise<void> }) => {
  ...
  // return in submit, that made "Create relationship" a no-op with no feedback.
  ...
  const submit = async () => { if (!organizationId || !form.first_name.trim() || !form.last_name.trim()) return; setBusy(true); const result = await database.rpc('create_crm_contact', { target_organization: organizationId, contact_kind: kind, ...form, target_project: projectId === 'none' ? null : projectId }); setBusy(false); if (result.error) { toast({ title: 'Relationship was not created', description: result.error.message, variant: 'destructive' }); return; } track('crm_contact_created', { contact_kind: kind, linked_to_project: projectId !== 'none' }); toast({ title: 'Relationship created' }); onOpenChange(false); await onCreated(); };
...
const Field = ({ label, children }: { label: string; children: React.ReactNode }) => <Label className="block space-y-2"><span className="block">{label}</span>{children}</Label>;
